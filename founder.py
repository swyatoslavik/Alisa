...
def get_city_coords(city):
    ...
    response = requests.get(api_url, params=params).json()
    return str(response['response']['GeoObjectCollection']['featureMember'][0]['GeoObject']['Point']['pos']).split()[::-1]
...
def get_hospital(city):
    ...
    if not response['features']:
        ...
        return response['features'][0]['geometry']['coordinates'], response['features'][0]['properties']['description']  # писок с 10 ближайшими объектами
